...
!function ($) {
	...
	var template = '<div class="pagination bPage bPageRight">' + 
		 ...
		 '<li id="bPageJumpNumber" class="bPageControlButton bPageInput"><a><input type="text" placeholder=" 输入页码"/></a></li>' +
		 '<li id="bPageJumpPage" class="bPageControlButton"><a href="javascript:void(0);">跳转</a></li>' +
		 '</ul>' + 
		 '</div>';
	/**
	 * 构造方法
	...
	bPage.prototype.populate = function(){
		...
		var pageInfo = '第 <strong>' + this.pageNumber + '</strong>/' + this.totalPage + ' 页' + '（总：<strong class="msg_red">'+this.totalRow+'</strong>）';
	...
	bPage.prototype.eventBind = function(){
		...
		$('#bPageJumpPage',$(this.$container)).on('click',function () {
			var pageNum = $('#bPageJumpNumber input').val();
			if(pageNum && Number(pageNum) > 0 && Number(pageNum) <= self.totalPage){
				self.pageSwitch(pageNum);
			}else {
				if(pageNum){
					window.alert("页码："+ pageNum +" 不在有效范围，总页数为："+self.totalPage);
					$('#bPageJumpNumber input').val("");
				}else {
					window.alert("请输入跳转的页码")
				}
			}
        });
	};
